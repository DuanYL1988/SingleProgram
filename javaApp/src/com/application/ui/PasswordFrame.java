...
import com.application.util.FileReadWriteUtil;
...
public class PasswordFrame extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent action) {
        ...
        if (db) {
            if ("查找".equals(kbn)) {
                ...
                if (bank_kbn) {
                    account = SecurityUtil.enCrypt(account);
                }
                resultText.append("");
            }
            // file mode
            ...
            if ("查找".equals(kbn)) {
                ...
                if (TextUtil.isNotEmpty(pswd)) {
                    ...
                    resultText.append("密码:" + SecurityUtil.deCrypt(pswd));
                } else {
                    // 未入力账户的情况下进行模糊查询
    ...
    private String validate(String action, String websit, String account, String password, boolean mode) {
        ...
        if ("查找".equals(action)) {
            ...
            if (!TextUtil.isNotEmpty(websit) && !TextUtil.isNotEmpty(account) && !TextUtil.isNotEmpty(password)) {
    ...
    private void setDefaultSelect(JRadioButton bank, JRadioButton notbank, JRadioButton file, JRadioButton database) {
        ...
        param_save = null == param_save ? "0" : param_save;
        if ("0".equals(param_save)) {
        ...
        param_bank = null == param_save ? "0" : param_save;
