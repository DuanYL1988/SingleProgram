...
public class ModelCreater extends BaseCreater {
    ...
    public String createModel(Table tableObj, String projectName, String pkgNm, boolean fileFlag, boolean hibernateFlag) {

        StringBuilder modelStr = new StringBuilder();
        pkgNm = pkgNm + ".model";
        modelStr.append("package " + pkgNm + ";" + Context.CRLF2);
        modelStr.append("import java.util.Date;" + Context.CRLF2);
        modelStr.append("import java.math.BigDecimal;" + Context.CRLF2);
        // hibernate
        if (hibernateFlag) {
        ...
        for (Table.Column field : tableObj.getColumnList()) {
            ...
            if (hibernateFlag) {
                if ("ID".equals(field.getColumnName())) {
                    ...
                    param.append(Context.SPACE4 + "@GeneratedValue(strategy = GenerationType.AUTO)" + Context.CRLF);
                }
                param.append(Context.SPACE4 + "@Column(name=\"" + field.getColumnName() + "\")" + Context.CRLF);
            }

            // 输出参数
            param.append(Context.SPACE4 + "private " + field.getJavaType() + " " + javaNm + ";" + Context.CRLF + Context.CRLF);

            // 输出set和get
            setAndGet.append(Context.SPACE4 + "/**" + Context.CRLF);
            setAndGet.append(Context.SPACE4 + " * 设定" + logicNm + Context.CRLF);
            setAndGet.append(Context.SPACE4 + " */" + Context.CRLF);
            setAndGet.append(Context.SPACE4 + "public void set" + metNm + "(" + type + " " + javaNm + "){" + Context.CRLF);
        ...
        if (fileFlag) {
            return super.writeFile(projectName, pkgNm, fileNm + ".java", modelStr.toString());
        }
