...
public class DaoCreater extends BaseCreater {
    ...
    private String getResultMapType(Table tableObj) {
        String modelNm = TextUtil.transNmDbToJava(tableObj.getName(), true);
        return modelNm + "ResultMap";
    }
     ...
     * @return
     */
    public String createMybatis(Table tableObj, String projectName, String pkgNm, Map<String, String> resultMap, boolean fileFlag) {
        // 纯java项目
        boolean javaProjFlag = "Y".equals(resultMap.get(Context.JAVA_FLAG));
        ...
        // Package
        String thisPkg = pkgNm + ".repository";
        daoStr.append("package " + pkgNm + ".repository;" + Context.CRLF2);
        ...
        daoStr.append("    " + modelNm + " selectOneById(@Param(\"id\")" + pkType + " id);" + Context.CRLF2);
        daoStr.append("    " + modelNm + " selectOneByUniqueKey(" + editUnionKey(tableObj) + ");" + Context.CRLF2);
        daoStr.append("    int getCountByDto(" + modelNm + " " + modelVarNm + ");" + Context.CRLF2);
        daoStr.append("    List<" + modelNm + "> selectByDto(" + modelNm + " " + modelVarNm + ");" + Context.CRLF2);
        daoStr.append("    int insert(" + modelNm + " " + modelVarNm + ");" + Context.CRLF2);
        daoStr.append("    int update(" + modelNm + " " + modelVarNm + ");" + Context.CRLF2);
        daoStr.append("    int delete(@Param(\"id\")" + pkType + " id);" + Context.CRLF2);
        daoStr.append("    List<" + modelNm + "> customQuary(" + modelNm + " " + modelVarNm + ");" + Context.CRLF2);
        daoStr.append("}" + Context.CRLF2);

        if (fileFlag) {
            return super.writeFile(projectName, thisPkg, fileNm + ".java", daoStr.toString());
        }
     ...
     * @return
     */
    public String createMappingXml(Table tableObj, String projectName, String pkgNm, Map<String, String> resultMap, boolean fileFlag) {
        boolean oracleFlag = "Y".equals(resultMap.get(Context.ORACLE));
        boolean springbootFlag = "Y".equals(resultMap.get(Context.SPRINGBOOT_FLAG));
        boolean javaFlag = "Y".equals(resultMap.get(Context.JAVA_FLAG));
        // 实体类
        String modelNm = TextUtil.transNmDbToJava(tableObj.getName(), true);
        String fileNm = modelNm + "Repository";
        ...
        xmlStr.append("<!DOCTYPE mapper PUBLIC \"-//MYBATIS.org//DTD Mapper 3.0//EN\" \"http://MYBATIS.org/dtd/MYBATIS-3-mapper.dtd\" >" + Context.CRLF);
        xmlStr.append("<mapper namespace=\"" + pkgNm + ".repository." + modelNm + "Repository\" >" + Context.CRLF);

        String paramStr = pkgNm + ".model." + modelNm;
        // resultMap
        xmlStr.append(createResultMap(tableObj, pkgNm));
        // select字段
        xmlStr.append(createSelectCol(tableObj, "select", oracleFlag));
        // insert字段
        xmlStr.append(createSelectCol(tableObj, "insert", oracleFlag));
        // selectOne
        xmlStr.append(createSelectOneById(tableObj));
        // countByDto
        xmlStr.append(createCountByDto(tableObj, paramStr));
        // selectByDto
        xmlStr.append(createSelectByDto(tableObj, paramStr, oracleFlag));
        // insert
        xmlStr.append(createInsert(tableObj, paramStr, oracleFlag));
        // delete
        xmlStr.append(createDelete(tableObj));
        // update
        ...
        if (fileFlag) {
            if (springbootFlag) {
                projectName = projectName.replace("java", "resources");
                pkgNm = "mapper";
            } else if (javaFlag) {
                pkgNm = pkgNm + ".repository";
            } else {
                pkgNm = pkgNm + ".mapping";
            }
            return super.writeFile(projectName, pkgNm, fileNm + ".xml", xmlStr.toString());
        }
    ...
    public String createHibernate(Table tableObj, String projectName, String pkgNm, boolean fileFlag) {
        ...
        repositoryStr.append("import org.springframework.data.jpa.repository.JpaRepository;" + Context.CRLF);
        repositoryStr.append("import org.springframework.data.repository.query.QueryByExampleExecutor;" + Context.CRLF);
        repositoryStr.append("import org.springframework.stereotype.Repository;" + Context.CRLF2);

        repositoryStr.append("@Repository" + Context.CRLF);
        repositoryStr.append("public interface " + fileNm + " extends JpaRepository<" + modelNm + ",String>,");
        repositoryStr.append(" QueryByExampleExecutor<" + modelNm + "> {" + Context.CRLF2);

        // 业务上查找一条
        repositoryStr.append(createHibernateCustomerSql(tableObj));

        repositoryStr.append("}" + Context.CRLF2);

        if (fileFlag) {
            return super.writeFile(projectName, thisPkg, fileNm + ".java", repositoryStr.toString());
        }
    ...
    private String createResultMap(Table table, String pkgNm) {
        ...
        resultMap.append("  <resultMap id=\"" + getResultMapType(table) + "\" type=\"" + pkgNm + ".model." + objNm + "\" >" + Context.CRLF);
     ...
     * select字段
     */
    private String createSelectCol(Table table, String colDiv, boolean oracleFlag) {
        ...
        for (Table.Column field : table.getColumnList()) {
            ...
            selectColumns.append(Context.SPACE4 + comma + (oracleFlag ? "" : "`") + field.getColumnName() + (oracleFlag ? "" : "`") + Context.CRLF);
        }
    ...
    private String createSelectOneById(Table table) {
        ...
        selectOne.append(Context.SPACE2 + "<select id=\"selectOneById\" resultMap=\"" + getResultMapType(table) + "\">" + Context.CRLF);
    ...
    private String createSelectOneByUniqueKey(Table table) {
        StringBuilder selectOne = new StringBuilder();
        selectOne.append(Context.SPACE2 + "<select id=\"selectOneByUniqueKey\" resultMap=\"" + getResultMapType(table) + "\">" + Context.CRLF);
        selectOne.append(Context.SPACE4 + "SELECT" + Context.CRLF);
        selectOne.append(Context.SPACE6 + "<include refid=\"select_column_list\"></include>" + Context.CRLF);
        selectOne.append(Context.SPACE4 + "FROM " + Context.CRLF);
        selectOne.append(Context.SPACE6 + table.getName() + Context.CRLF);
        selectOne.append(Context.SPACE4 + "<where>" + Context.CRLF);
        selectOne.append(Context.SPACE4 + "  1 = 1" + Context.CRLF);
        for (Table.Column field : table.getUniqueKey()) {
            selectOne.append(Context.SPACE4 + Context.SPACE4 + "AND" + " " + field.getColumnName() + " = " + "#{" + field.getSourceName() + "}" + Context.CRLF);
        }
        selectOne.append(Context.SPACE4 + "</where>" + Context.CRLF);
        selectOne.append(Context.SPACE2 + "</select>" + Context.CRLF + Context.CRLF);
        return selectOne.toString();
     ...
     * @return
     */
    private String createHibernateCustomerSql(Table table) {
        String pojoNm = TextUtil.transNmDbToJava(table.getName(), true);
        StringBuilder selectOne = new StringBuilder();
        selectOne.append("    /**" + Context.CRLF);
        selectOne.append("     * 使用规定方法名查找业务上唯一一件" + Context.CRLF);
        selectOne.append("     */" + Context.CRLF);

        selectOne.append("    " + pojoNm + " findFirstBy");
        StringBuilder paramterStr = new StringBuilder();
        int index = 0;
        for (Table.Column field : table.getUniqueKey()) {
            String colNm = field.getColumnName();
            if (index > 0) {
                selectOne.append("And");
                paramterStr.append(",");
            }
            selectOne.append(TextUtil.transNmDbToJava(colNm, true));
            paramterStr.append(field.getJavaType() + " " + TextUtil.transNmDbToJava(colNm, false));
            index++;
        }
        selectOne.append("(" + paramterStr + ");" + Context.CRLF2);
        return selectOne.toString();
    }

    private String createCountByDto(Table table, String parameterType) {
        StringBuilder getCountByDto = new StringBuilder();
        getCountByDto.append(Context.SPACE2 + "<select id=\"getCountByDto\" parameterType=\"" + parameterType + "\" resultType=\"java.lang.Integer\">" + Context.CRLF);
        getCountByDto.append(Context.SPACE4 + "SELECT COUNT(*) FROM " + table.getName() + Context.CRLF);
        getCountByDto.append(Context.SPACE4 + "<where>" + Context.CRLF);
        getCountByDto.append(Context.SPACE6 + "1 = 1" + Context.CRLF);
        getCountByDto.append(createColCheck(table.getColumnList(), "select"));
        getCountByDto.append(Context.SPACE6 + "<if test=\"condition!=null and condition!=''\">" + Context.CRLF);
        getCountByDto.append(Context.SPACE4 + Context.SPACE4 + "${condition}" + Context.CRLF);
        getCountByDto.append(Context.SPACE6 + "</if>" + Context.CRLF);
        getCountByDto.append(Context.SPACE4 + "</where>" + Context.CRLF);
        getCountByDto.append(Context.SPACE2 + "</select>" + Context.CRLF + Context.CRLF);
        return getCountByDto.toString();
    }

    /**
     * 复杂查询
     */
    private String createSelectByDto(Table table, String parameterType, boolean oracleFlag) {
        StringBuilder selectByDto = new StringBuilder();
        selectByDto.append(Context.SPACE2 + "<select id=\"selectByDto\" parameterType=\"" + parameterType + "\" resultMap=\"" + getResultMapType(table) + "\">" + Context.CRLF);
        if (oracleFlag) {
            selectByDto.append(Context.SPACE4 + "SELECT * FROM (");
        }
        ...
        if (oracleFlag) {
            selectByDto.append(Context.SPACE6 + ", ROWNUM AS ROW_NO ");
        }
        selectByDto.append(Context.SPACE4 + "FROM " + (oracleFlag ? "( SELECT * FROM " : "") + Context.CRLF);
        selectByDto.append(Context.SPACE6 + table.getName() + Context.CRLF);
        selectByDto.append(Context.SPACE4 + "<where>" + Context.CRLF);
        selectByDto.append(Context.SPACE6 + "1 = 1" + Context.CRLF);
        ...
        selectByDto.append(Context.SPACE6 + "ORDER BY ${orderBy}" + Context.CRLF);
        selectByDto.append(Context.SPACE4 + "</if>" + Context.CRLF);
        if (!oracleFlag) {
            selectByDto.append(Context.SPACE6 + "LIMIT ${startRow} , ${pageSize}" + Context.CRLF);
        }
        if (oracleFlag) {
            selectByDto.append(Context.SPACE6 + ") )WHERE ROW_NO BETWEEN (${startRow} + 1) AND (${startRow} + ${pageSize})" + Context.CRLF);
        }
    ...
    private String createUpdate(Table table, String parameterType) {
        ...
        update.append(Context.SPACE2 + "<update id=\"update\" parameterType=\"" + parameterType + "\" >" + Context.CRLF);
        ...
        update.append(Context.SPACE6 + pkCol.getColumnName() + " = " + "#{" + pkCol.getSourceName() + "}" + Context.CRLF);
     ...
     * 更新
     */
    private String createDelete(Table table) {
        StringBuilder delete = new StringBuilder();
        delete.append(Context.SPACE2 + "<delete id=\"delete\">" + Context.CRLF);
        delete.append(Context.SPACE4 + "DELETE FROM " + table.getName() + " WHERE ");
        Table.Column pkCol = table.getPkey();
        delete.append(pkCol.getColumnName() + " = " + "#{" + pkCol.getSourceName() + "}" + Context.CRLF);
        delete.append(Context.SPACE2 + "</delete>" + Context.CRLF + Context.CRLF);
     ...
     * 追加
     */
    private String createInsert(Table table, String parameterType, boolean oracleFlag) {
        String tableNm = table.getName();
        StringBuilder insert = new StringBuilder();
        insert.append("  <insert id=\"insert\" parameterType=\"" + parameterType + "\" >" + Context.CRLF);
        // Oracle中用sequence
        if (oracleFlag) {
            insert.append(Context.SPACE4 + Context.SPACE4 + "<selectKey resultType=\"java.lang.Integer\" order=\"BEFORE\" keyProperty=\"id\">" + Context.CRLF);
        ...
        if (!oracleFlag) {
            insert.append(Context.SPACE4 + ") VALUES (" + Context.CRLF);
        } else {
        ...
        for (Table.Column field : table.getColumnList()) {
            ...
            insert.append(Context.SPACE6 + comma + " #{" + field.getSourceName());
            if (oracleFlag) {
                insert.append(",jdbcType = " + field.getColumnType());
            }
            insert.append("}" + Context.CRLF);
        }
        ...
        if (oracleFlag) {
            ...
            insert.append(Context.SPACE4 + ")" + Context.CRLF);
        }
    ...
    private String createCustomQuery(Table table, String tableNm, String paramStr) {
        ...
        custom.append("  <select id=\"customQuary\" parameterType=\"" + paramStr + "\" resultType=\"" + paramStr + "\">" + Context.CRLF);
    ...
    private void appendCheckCols(StringBuilder condition, Table.Column field, String mark) {
        ...
        if ("AND".equals(mark)) {
            condition.append(Context.SPACE4 + Context.SPACE4 + mark + " " + field.getColumnName() + " = " + "#{" + javaNm + "}" + Context.CRLF);
        } else if (",".equals(mark)) {
            condition.append(Context.SPACE4 + Context.SPACE4 + field.getColumnName() + " = " + "#{" + javaNm + "}" + mark + Context.CRLF);
        }
