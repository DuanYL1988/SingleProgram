    ...
    public void initFile(String filePath) {
        ...
        try {
            if (!localFile.exists()) {
                ...
                pw = new OutputStreamWriter(new FileOutputStream(filePath), Code.CHARSET_CODE_UTF8);
                pw.close();
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public String getData(String websit, String path, boolean bank_kbn) {
        ...
        if (resultList.size() == 0) {
            ...
            for (String result : resultList) {
                ...
                String result_account = result.substring(0, result.length() - 25);
                String result_password = result.substring(result.length() - 24, result.length());
                // 模糊查询时不解密账号
                if (bank_kbn) {
                    resultText.append("\n账号(卡号):" + SecurityUtil.deCrypt(result_account));
                } else {
                    resultText.append("\n账号(卡号):" + result_account);
                }
                resultText.append("\n密码:" + SecurityUtil.deCrypt(result_password) + "\n");
            }
        }
